...
// This example reads values from the switch and send the 
// read value to the ARM
object SwitchHostIO extends SpatialApp {
  ...
  def main() { 
    ...
    Accel(*) {
      // Read from switch and send the value to hostIO
...
// This app tests accessing a 1-D memory
object MemTest1D extends SpatialApp {
  ...
  def main() {
    ...
    // Create HW accelerator
    Accel {
      // Creating a 1-D memory
      val mem = SRAM[Int](384)
      Sequential.Foreach(384 by 1) { i =>
        // Set the memory
        mem(i) = x + i.to[Int]
      }
      // Set the argOut to a value stored in the memory
      Pipe { y := mem(383) }
    }
...
// This app tests accessing a 2-D memory
