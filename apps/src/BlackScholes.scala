...
//   @virtualize
//   def BlkSchlsEqEuroNoDiv(sptprice: T, strike: T, rate: T, volatility: T, time: Float, otype: Int) = {
//     val xLogTerm = log( sptprice / strike )
//     val xPowerTerm = (volatility ** 2) * 0.5.as[T]
//     val xNum = (rate + xPowerTerm) * time.as[T] + xLogTerm
//     val xDen = volatility * sqrt(time).as[T]
...
//     val ftime  = Array.fill(N)(random[Float])
