...
object PageRank_1_7680_0d125_ip_16_ts_768 extends SpatialApp { // DISABLED Regression (Sparse) // Args: 1 768 0.125
  import IR._
...
val ip = 16
    ...
    Accel {
      val frontierOff = SRAM[Int](ts)
      val currentPR = SRAM[T](ts)
      // Flush frontierOff so we don't cause gather segfault. Flush currentPR because mux isn't do what I thought it would
      Foreach(ts by 1) { i => frontierOff(i) = 0.to[Int]; currentPR(i) = 1.to[T]}

      Sequential.Foreach(iters by 1){ iter =>
        // val oldPrIdx = iter % 2.as[SInt]
        // val newPrIdx = mux(oldPrIdx == 1, 0.as[SInt], 1.as[SInt])
        Sequential.Foreach(NP by ts) { tid =>
          val initPR = SRAM[T](ts)

          val edgesId = SRAM[Int](ts)
          val edgesLen = SRAM[Int](ts)
          Parallel {
            initPR load OCpages(tid::tid+ts)
            edgesId load OCedgeId(tid :: tid+ts)
            edgesLen load OCedgeLen(tid :: tid+ts)
          }

          Sequential.Foreach(ts by 1) { pid =>
            ...
            val edges = SRAM[Int](ts)
            val counts = SRAM[T](ts)
            Parallel {
            ...
            val offLoc = SRAM[Int](ts)
            val onChipMask = SRAM[Int](ts) // Really bitmask
            ...
            Sequential.Foreach(numEdges.value by 1){ i =>
              ...
              val onchip = addr >= tid && addr < tid+ts
              offAddr := offAddr.value + mux(onchip, 0, 1)
              offLoc(i) = mux(onchip, offAddr.value, (ts-1).to[Int]) // Probably no need to mux here
            ...
            // Compute new PR
            val pr = Reduce(Reg[T])(numEdges.value by 1 par ip){ i =>
              val addr = edges(i)
              val off  = offLoc(i)
          ...
          OCpages(tid::tid+ts) store currentPR
        }
      }
    }
    getMem(OCpages)
