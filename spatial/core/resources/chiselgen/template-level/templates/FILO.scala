...
class FILO(val pR: Int, val pW: Int, val depth: Int, val bitWidth: Int = 32) extends Module {
  ...
  subAccessor.io.input.max := (p/min(pW,pR)).S
  subAccessor.io.input.enable := io.push | io.pop
  subAccessor.io.input.stride := Mux(io.push, 1.S, -1.S)
  ...
  accessor.io.input.enable := (io.push & subAccessor.io.output.done) | (io.pop & subAccessor.io.output.count(0) === 0.S)
  ...
  if (pW == pR) {
    ...
    (0 until pW).foreach { w_i => 
      (0 until (p / pW)).foreach { i => 
        ...
        m(w_i + i*pW).io.w.en := io.push & (subAccessor.io.output.count(0) === i.S)
      }
    }
  }
  ...
  if (pW == pR) {
    ...
    (0 until pR).foreach { r_i => 
      ...
      (0 until (p / pR)).foreach { i => 
        ...
        m(r_i + i*pR).io.r.en := io.pop & (subAccessor.io.output.count(0) === i.S)
