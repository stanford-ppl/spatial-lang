...
module  cl_tst #(parameter DATA_WIDTH=512, parameter NUM_RD_TAG=512) (
   ...
   output logic DIRECT_rdata_valid,
...
always @(posedge clk)
   begin
      if (rd_dat_ram_wr)
      begin
        $display("------------ LOAD DDR -> BRAM ------------");
        $display("rd_dat_ram_addr = 0x%x", rd_dat_ram_addr);
        $display("rdata_q = 0x%x", rdata_q);
      end
   end
*/

assign DIRECT_rdata_valid = rd_dat_ram_wr;
assign DIRECT_rdata = rdata_q;
