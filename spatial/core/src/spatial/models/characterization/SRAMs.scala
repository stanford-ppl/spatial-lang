...
trait SRAMs extends Benchmarks {
  ...
  case class SRAM1DOp[T:Type:Bits](depth: scala.Int, len: scala.Int, p: scala.Int)(val N: scala.Int) extends Benchmark {
    val prefix: String = s"${depth}_${len}_${p}"
    def eval(): Unit = {
  ...
  val dims1d = List(512, 1024, 2048, 4096, 8192, 16384, 32768, 65536)
  val pars = List(1, 2, 4)
