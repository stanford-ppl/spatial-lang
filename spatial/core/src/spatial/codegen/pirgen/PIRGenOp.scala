          ...
          case FixConvert(x) => 
            ...
            lhs.tp match {
              ...
              case FixPtType(s2, i2, f2) =>
                val bitWidth = i2 + f2
                if (bitWidth != 32) {
                  warn(s"Plasticine only support 32 bit wordwidth. FixConvert to ${lhs.tp}, bitWidth=$bitWidth ${lhs.ctx}")
                  dbg(s"Plasticine only support 32 bit wordwidth. FixConvert to ${lhs.tp}, bitWidth=$bitWidth ${lhs.ctx}")
                }
                /*
                 * [ s1 | i1 |   f1 ]
                 * [ s2 | i2   | f2 ]
