...
class PIRAllocation(implicit val codegen:PIRCodegen) extends PIRTraversal {
  ...
  def allocateMemoryCU(dmem: Expr):List[CU] = {
    val cus = mappingOf.getOrElseUpdate(dmem) { 
      ...
      dbgblk(s"allocateMemoryCU ${qdef(mem)}") {
        ...
        mutable.Set() ++ {
          val m = instanceOf(mem)
          val numBanks = outerDimsOf(mem).map{dim => m.banking.find(_.dims.contains(dim)).get.nBanks }.product
          (0 until numBanks).map { bank =>
            val cu = ComputeUnit(s"${quote(dmem)}_bank${bank}", MemoryCU)
            dbgs(s"Allocating MCU duplicates $cu for ${quote(dmem)}, bank $bank")
            cu.parent = parentCU
            val pmem = createSRAM(dmem, m, cu)
            bankOf(pmem) = bank
            instOf(pmem) = 0
            cu
          }
    ...
    val cuMem = getOrElseUpdate(cu.memMap, dmem, {
      ...
      mem match {
        case mem if isReg(mem) => //TODO: Consider initValue of Reg?
          ...
          cuMem.bufferDepth = Some(getDuplicate(dmem, dreader).depth)
  ...
  def extractAddrExprs(access:Expr) = {
    ...
    val inds = access match {
      case BankedReader((_, Some(addrs),_, _)::_) => addrs.head // Assume SIMD
      case BankedWriter((_, _, Some(addrs),_, _)::_) => addrs.head
      case _ => Nil
    }
  ...
  override protected def visit(lhs: Sym[_], rhs: Op[_]) = {
    dbgl(s"Visiting ${qdef(lhs)}") {
      rhs match {
        ...
        case BankedWriter((mem, _,_, _, _)::_)  =>
