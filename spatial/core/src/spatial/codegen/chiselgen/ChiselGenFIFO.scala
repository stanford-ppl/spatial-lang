...
trait ChiselGenFIFO extends ChiselCodegen {
  ...
  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    ...
    case FIFOEnq(fifo,v,en) => 
      ...
      emit(src"""${fifo}.io.enq := ShiftRegister($enabler & ~${writer}_inhibitor, ${symDelay(en)}) & $en """)
    ...
    case FIFODeq(fifo,en) =>
      ...
      emit(src"""${fifo}.io.deq := ShiftRegister(${reader}_datapath_en & ~${reader}_inhibitor, ${symDelay(en)}) & $en & ~${reader}_inhibitor""")
