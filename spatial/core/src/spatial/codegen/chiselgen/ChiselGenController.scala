            ...
            (start, end, step, par) match {
              ...
              */
              ...
              case (Exact(s), _, Exact(st), Exact(p)) => 
                ...
                emit(src"val ${sym}${i}_jump = ${st} * ${p}")
                emit(src"val ${sym}${i}_hops = Utils.getRetimed(${sym}${i}_range /-/ ${sym}${i}_jump.FP(true, 32, 0), Utils.fixdiv_latency)")
                emit(src"val ${sym}${i}_leftover = Utils.getRetimed(${sym}${i}_range %-% ${sym}${i}_jump.FP(true, 32, 0), Utils.fixmod_latency)")
              ...
              case (Exact(s), Exact(e), _, Exact(p)) => 
                ...
                emit(src"val ${sym}${i}_range = ${e} - ${s}")
                emit(src"val ${sym}${i}_jump = Utils.getRetimed(${step} *-* ${p}.S(${w}.W), Utils.fixmul_latency)")
                emit(src"val ${sym}${i}_hops = Utils.getRetimed((${sym}${i}_range.S(${w}.W) /-/ ${sym}${i}_jump).asUInt, Utils.fixdiv_latency)")
                emit(src"val ${sym}${i}_leftover = Utils.getRetimed(${sym}${i}_range.S(${w}.W) %-% ${sym}${i}_jump, Utils.fixmod_latency)")
              ...
              case _ => 
                emit(src"val ${sym}${i}_range = Utils.getRetimed(${end} - ${start}, Utils.fixsub_latency)")
                emit(src"val ${sym}${i}_jump = Utils.getRetimed(${step} *-* ${par}, Utils.fixmul_latency)")
                emit(src"val ${sym}${i}_hops = Utils.getRetimed(${sym}${i}_range /-/ ${sym}${i}_jump, Utils.fixdiv_latency)")
                emit(src"val ${sym}${i}_leftover = Utils.getRetimed(${sym}${i}_range %-% ${sym}${i}_jump, Utils.fixmod_latency)")
                emit(src"val ${sym}${i}_evenfit = Utils.getRetimed(${sym}${i}_leftover === 0.U, Utils.fixeql_latency)")
                emit(src"val ${sym}${i}_adjustment = Utils.getRetimed(Mux(${sym}${i}_evenfit, 0.U, 1.U), Utils.mux_latency)")
                emitGlobalWireMap(src"${sym}_level${i}_iters", src"Wire(UInt(32.W))")
                emit(src"""${swap(src"${sym}_level${i}_iters", Blank)} := Utils.getRetimed(${sym}${i}_hops + ${sym}${i}_adjustment, Utils.fixadd_latency).r""")
