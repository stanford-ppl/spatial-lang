            ...
            (start, end, step, par) match {
              ...
              */
              ...
              case (Exact(s), Exact(e), _, Exact(p)) => 
                ...
                emitGlobalWireMap(src"${sym}_level${i}_iters", src"Wire(UInt(${32 min 2*w}.W))")
                emit(src"""${swap(src"${sym}_level${i}_iters", Blank)} := ${DL(src"${sym}${i}_hops + ${sym}${i}_adjustment", s"(Utils.fixadd_latency*${sym}${i}_hops.getWidth).toInt")}.r""")
              case (_, _, Exact(st), Exact(p)) => 
                appPropertyStats += HasVariableCtrBounds
                emit(src"val ${sym}${i}_range =  ${DL(src"${end} - ${start}", s"(Utils.fixsub_latency*${end}.getWidth).toInt")}")
                emit(src"val ${sym}${i}_jump = ${st} * ${p}")
