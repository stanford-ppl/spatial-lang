    ...
    if (SpatialConfig.enableRetiming || SpatialConfig.enablePIRSim) {
      ...
      if (fsm.isDefined) {
        ...
        if (cchain.isDefined) {
          ...
          val rster = if (levelOf(lhs) == InnerControl & listensTo(lhs).distinct.length > 0) {src"Utils.risingEdge(${swap(lhs, Done)}).D(1 + ${lhs}_retime, rr) // Ugly hack, do not try at home"} else src"${lhs}_done.D(1, rr)"
