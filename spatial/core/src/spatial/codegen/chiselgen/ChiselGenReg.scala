    ...
    case RegWrite(reg,v,en) => 
      ...
      if (isArgOut(reg) | isHostIO(reg)) {
        val id = argMapping(reg).argOutId
          ...
          emit(src"""${swap(reg, EnOptions)}(${lhs}_wId) := $en & Utils.getRetimed(${swap(parent, DatapathEn)}, ${enableRetimeMatch(en, lhs)}.toInt)""")
      } else {
        reduceType(lhs) match {
              ...
              fps match {
                case FixPtSum =>
                  if (dup.isAccum) {
                    ...
                    emit(src"""${swap(src"${reg}_${ii}", Blank)}.io.input.enable := ${swap(reg, Wren)}.D(${enableRetimeMatch(en, lhs)}.toInt)""")
                    emit(src"""${swap(src"${reg}_${ii}", Blank)}.io.input.init := ${reg}_initval.number""")
                    emit(src"""${swap(src"${reg}_${ii}", Blank)}.io.input.reset := reset.toBool | (${swap(reg, Resetter)} ${manualReset}).D(${enableRetimeMatch(en, lhs)}.toInt, rr)""")
                    ...
                    emit(src"""${swap(src"${reg}_${ii}", Blank)}.write(${data_string}, $en & (${swap(reg, Wren)} & ${swap(parent, IIDone)}).D(${enableRetimeMatch(en, lhs)}.toInt+1), reset.toBool ${manualReset}, List($ports), ${reg}_initval.number, accumulating = ${isAccum(lhs)})""")
                  }
                case _ =>
                  val ports = portsOf(lhs, reg, ii) // Port only makes sense if it is not the accumulating duplicate
                  val dlay = if (accumsWithIIDlay.contains(reg)) {src"${reg}_II_dlay"} else "0" // Ultra hacky
                  if (dup.isAccum) {
                    emit(src"""${swap(src"${reg}_${ii}", Blank)}.write($v, $en & (${swap(reg, Wren)} & ${swap(parent, IIDone)}.D($dlay)).D(${enableRetimeMatch(en, lhs)}.toInt), reset.toBool | ${swap(reg, Resetter)}.D(${enableRetimeMatch(en, lhs)}.toInt) ${manualReset}, List($ports), ${reg}_initval.number, accumulating = ${isAccum(lhs)})""")
                  } else {
                    emit(src"""${swap(src"${reg}_${ii}", Blank)}.write($v, $en & (${swap(reg, Wren)} & ${swap(parent, IIDone)}.D($dlay)).D(${enableRetimeMatch(en, lhs)}.toInt), reset.toBool ${manualReset}, List($ports), ${reg}_initval.number, accumulating = ${isAccum(lhs)})""")
                  }
          ...
          case _ => // Not an accum
            duplicatesOf(reg).zipWithIndex.foreach { case (dup, ii) =>
              val ports = portsOf(lhs, reg, ii) // Port only makes sense if it is not the accumulating duplicate
              emit(src"""${swap(src"${reg}_${ii}", Blank)}.write($v, $en & (${swap(parent, DatapathEn)} & ${swap(parent, IIDone)}).D(${enableRetimeMatch(en, lhs)}.toInt), reset.toBool ${manualReset}, List($ports), ${reg}_initval.number, accumulating = ${isAccum(lhs)})""")
            }
        }
      }
