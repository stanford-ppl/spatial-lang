...
void handleDRAMRequest(DUT *dut, PeekPokeTester *tester) {
  ...
  if (isWr) {
    ...
    tester->poke(&(dut->io_dram_resp_bits_rdata_0), raddr[0 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_1), raddr[1 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_2), raddr[2 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_3), raddr[3 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_4), raddr[4 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_5), raddr[5 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_6), raddr[6 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_7), raddr[7 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_8), raddr[8 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_9), raddr[9 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_10), raddr[10 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_11), raddr[11 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_12), raddr[12 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_13), raddr[13 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_14), raddr[14 + 8]); // No idea why we need to add 8...
    tester->poke(&(dut->io_dram_resp_bits_rdata_15), raddr[15 + 8]); // No idea why we need to add 8...
  }
