...
class TmpCounter(max: Int, reset_val: Int, stride: Int = 1) extends Module {
  val io = IO(new Bundle {
    ...
    val en    = Input(Bool())
    val count = Output(UInt(log2Up(max+stride).W))
  ...
  val x = Reg(init=reset_val.asUInt(log2Up(max+stride).W))
  when(io.reset) {
    x := reset_val.U(log2Up(max+stride).W)
  } .elsewhen(io.en) {
...
class LineBuffer(val num_lines: Int, val line_size: Int, val extra_rows_to_buffer: Int, val COL_PAR: Int, val ROW_PAR: Int) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val col_addr = Input(UInt(log2Up(line_size+1).W)) // From each row, read COL_PAR px starting at this col_addr
  ...
  //                val initializing_state = Reg(init=reset_val.asUInt(log2Up(max+1).W))
  //                ...
