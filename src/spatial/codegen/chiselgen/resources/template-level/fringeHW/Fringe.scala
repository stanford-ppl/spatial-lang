...
class Fringe(
  ...
  val io = IO(new Bundle {
    ...
    val genericStreamOutTop = Flipped(Decoupled(new StreamOutAccel(StreamParInfo(w, 1))))
    val genericStreamInTop = Decoupled(new StreamInAccel(StreamParInfo(w, 1)))
  ...
  if (io.genericStreamsAccel.outs.length == 1) {
    io.genericStreamsAccel.outs(0) <> io.genericStreamInTop    
  }
  if (io.genericStreamsAccel.ins.length == 1) {
    io.genericStreamsAccel.ins(0) <> io.genericStreamOutTop
  }
}
