...
" >> $1
  ...
  if [[ ${type_todo} = "scala" ]]; then
    echo "# Compile app
${SPATIAL_HOME}/bin/spatial --scala --emission=1 --outdir=${SPATIAL_HOME}/regression_tests/${2}/${3}_${4}/out ${4} ${args} 2>&1 | tee -a ${5}/log
    " >> $1
  elif [[ ${type_todo} = "chisel" ]]; then
    echo "# Compile app
${SPATIAL_HOME}/bin/spatial --chisel --emission=1 --outdir=${SPATIAL_HOME}/regression_tests/${2}/${3}_${4}/out ${4} 2>&1 | tee -a ${5}/log
    " >> $1
  fi
